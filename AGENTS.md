...
- `infra` … Terraform（envs = local / prod。prodは未着手）
...
- **委譲後は必ず Claude Code 側で検収する**: diff を `/code-review` でレビューし、typecheck・実機確認
  （ローカル起動・curl・ブラウザ）を通す。問題があれば `--resume` で修正指示を返す。
  レビューは Claude Code の担当（`/codex:review` ではなく `/code-review` を使う）。
  **レビューのeffortはdiff規模に合わせる**: 小さいdiff（目安200行未満）は low〜medium、
  high以上はリスクの高い変更（認証・課金・データ破壊系）や大規模変更に限る。
