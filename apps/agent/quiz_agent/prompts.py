...
#
# 2026-08-09 の計測(n=30)で、旧文面の「原文に根拠のない主張は削って、問題と解説を
# 作り直してください」が実害を出していたため書き換えた。実測:
# - 再生成11回のうち設問が書き換わったのが10回。正常だった設問が「問いが無い」状態に
#   転じたのが3回で、うち2回はそのままゲートを通過した(通過3回中2回が壊れた問題)
# - 451字のシナリオ設問が91字になり、要件・症状・問いが消えて回答不能になった例あり
# 原因は、シナリオ(状況設定・要件・症状)こそドキュメント原文に無い部分であり、
# 「根拠のない主張を削る」と最も削られるのがそこだったこと。スコアは上がるが問題は壊れる。
#
# ゲートが採点するのは grounding_claim_en と正解選択肢の本文だけなので、
# 書き直す範囲をそこに限定し、設問の保護を明示する。
QUIZ_REGENERATE_FEEDBACK_PROMPT = (
    "先ほど作成した問題は、グラウンディングチェック(調査したAWS公式ドキュメント原文への"
    "根拠づけの確認)でブロックされました。"
    "評価の対象は評価専用のgrounding_claim_enと正解選択肢の本文だけです。"
    "grounding_claim_enを、調査したドキュメント原文の記述に忠実な英語の主張文(2〜4文)"
    "として書き直してください。原文の逐語コピーではなく、原文に基づいて言い換えます。"
    "設問・選択肢・利用者向けの解説は維持し、短くしたり削ったりしないでください。"
    "特に設問のシナリオ(状況設定、要件の列挙、症状の記述、末尾の問いかけ)は、"
    "ドキュメント原文に書かれていなくて当然の情報です。原文に根拠が無いことを理由に"
    "削ってはいけません。設問から要件や問いかけが失われると、回答不能な問題になります。"
    "設問、選択肢、利用者向けの解説は日本語のプレーンテキストで記述し、HTMLタグや"
