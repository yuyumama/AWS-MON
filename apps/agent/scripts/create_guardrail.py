...
**このスクリプトは生成パスからは使われていない。**ADR 0018 で Guardrails
グラウンディングゲートを撤去したため、agent は ApplyGuardrail を呼ばない。
方針が変わったときのために残してある。

作成後に表示される guardrailId は、かつて .env の AGENT_GUARDRAIL_ID に設定すると
生成パスでグラウンディングチェックが有効になるものだった(現在その読み取り先は無い)。
