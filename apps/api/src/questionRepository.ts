...
function parseOptions(value: unknown): Option[] {
	if (!Array.isArray(value) || value.length < 2) {
		throw new ApiError(
			"quiz.question.options must include at least 2 options",
			400,
		);
	}
...
function parseCorrect(
	...
	if (type === "single" && correct.length !== 1) {
		throw new ApiError(
			"single question must have exactly 1 correct answer",
			400,
		);
	}
	...
	for (const label of correct) {
		if (!optionLabels.has(label)) {
			throw new ApiError(
				`correct answer label is not in options: ${label}`,
				400,
			);
		}
	}
...
function parseOptionReasons(
	...
	return value.map((raw, index) => {
		...
		if (!optionLabels.has(label)) {
			throw new ApiError(
				`option reason label is not in options: ${label}`,
				400,
			);
		}
...
function parseQuality(value: unknown): QualityInput {
	...
	const inlineGate =
		raw.inlineGate === "passed" || raw.inlineGate === "failed"
			...
			: "not_run";
	const evaluator =
		raw.evaluator === "self_review" || raw.evaluator === "agentcore_evaluate"
			? raw.evaluator
			: "none";
...
export async function saveGeneratedQuestion(
	...
	const listKeys = questionListKeys({ createdAt, questionId });
