...
function friendlyAuthError(error: unknown): Error {
  ...
  const message = (error as { message?: string })?.message ?? "認証に失敗しました";
  switch (name) {
    case "NotAuthorizedException":
    case "UserNotFoundException":
      return new Error("メールアドレスまたはパスワードが正しくありません。");
    case "PasswordResetRequiredException":
      return new Error("パスワードのリセットが必要です。管理者に連絡してください。");
    case "InvalidPasswordException":
      return new Error(`パスワードがポリシーを満たしていません: ${message}`);
    case "LimitExceededException":
    case "TooManyRequestsException":
      return new Error("試行回数が上限に達しました。しばらく待ってから再試行してください。");
    default:
