...
## 背景
モデル出力をJSON文字列として受け取り、末尾のカンマ除去などの後処理でパースする方式は壊れやすい。
...
- プロンプト（`prompts.py`）からJSON形状の指示を除去し、内容面の指示だけを残した。

## 根拠・影響
- Bedrock / Claude Sonnet 4.6 で tool use 経由で動作することを確認済み（インストール済みStrandsに `structured_output` が存在）。
- パース起因のバグが解消したため、`parsing.py` を削除した。正誤判定に必要な `arrays_equal` のみ `grading.py` に残した。
