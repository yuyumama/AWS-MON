...
## 決定
- **役割ベースのモノレポ**: `apps/`（動くもの）/ `infra/`（IaC）/ `local/`（ローカル環境）/ `docs/`（設計メモ）。AIが名前から「どこを触るか」を判断できることを優先する。
- `apps/` は `web` / `api` / `agent` とする。既存 `agent/` は `apps/agent/` へ移動する。
- **IaCは Terraform**。`infra/envs` は **local / prod の2つのみ**とし、ディレクトリで分割して管理する（個人開発のため、多環境化しない）。
...
## 根拠
- 個人開発のため、過剰な多環境・多パッケージを避け、必要になってから追加する方針である。
