...
2段生成に分ける主な目的は、「問題を先に速く出し、回答中に解説をバックグラウンドで作る」というレイテンシの隠蔽だった。
...
- 呼び出し1回により、**コスト削減**（2回目に問題文を入力として再送しない）・**実装の単純化**（部分状態と補完処理が不要）・**レイテンシ削減**（往復1回）を実現できる。
- 同一コンテキストで問題・正解・解説を確定するため、**内部整合性が向上する**（解説が問題を読み違えにくい）。

## トレードオフ / 留意
- 品質ゲートで問題だけ作り直したいケースでは、解説分も一緒に再生成になる。許容する（評価は生成後のレビュー段で担う）。
- 「問題表示→回答→その後に解説を出す」UXでも、保存済みの `QuizItem` から回答後に解説だけを返せばよいため、分離は不要である。
- 出力は1問分の問題＋解説であり、トークン上限には余裕がある。
