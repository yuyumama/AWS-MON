...
回数ではなく1トレースあたりのペイロードが巨大なことが費用の主因で、ジャッジが
...
たまらない）では自動・継続の品質採点に見合う便益がない。

## 決定

監視3層のうち**「品質（傾向）」層＝AgentCore Evaluations オンライン評価を廃止**し、
品質担保をインラインの Guardrails グラウンディングゲート（`quiz_agent/guardrail.py`、
...
- ADR 0010 が挙げた「閾値緩和の影響を Evaluations（Correctness）の傾向で継続観測する」
  ...
  （課金は都度・明示的）で代替する。
- 将来利用頻度が上がって傾向監視が要るようになった場合は、オンライン評価を
