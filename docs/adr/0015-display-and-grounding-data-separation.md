...
| モデル | コード | grounding単軸<br>通過率<br>(スコア取得分) | **実際の通過率**<br>**(status=passed)** | 初回通過率<br>(全サンプル) | grounding<br>中央値 | grounding<br>平均 | 所要<br>中央値 |
|---|---|---:|---:|---:|---:|---:|---:|
| nemotron-3-ultra:free | 変更前 | 63.3% (19/30) | **60.0%** | 63.3% | 0.74 | 0.63 | 151s |
| nemotron-3-ultra:free | **変更後** | **69.0% (20/29)** | **65.5%** | 66.7% | 0.73 | 0.68 | 146s |
| ling-3.0-flash:free | 変更前 | 65.5% (19/29) | **62.1%** | 63.3% | 0.73 | 0.67 | 37s |
| ling-3.0-flash:free | **変更後** | **82.1% (23/28)** | **67.9%** | 76.7% | 0.83 | 0.76 | 54s |

> **訂正（2026-08-09, issue #143）**: 「grounding単軸通過率」列は `grounding >= 0.6` を満たした割合であり、
> ゲートの合否ではない。ゲートは grounding と relevance の AND で判定するため、実際に通過した割合は
> `status=passed` 列（[ADR 0018](0018-retire-grounding-gate-as-quality-judge.md) の再集計値）である。
> 当初この列は「初回通過率」と書かれていた。**今後の計測では `status=passed` ベースを正とする。**

**結論: 分離によってグラウンディングは悪化せず、むしろ改善した。この結論は訂正後も変わらない。**
同一モデル（nemotron）で 63.3% → 69.0%（`status=passed` では 60.0% → 65.5%）、
ling-3.0-flash では 65.5% → 82.1%（同 62.1% → 67.9%）。

ただし**改善幅は指標によって大きく違う**。ling-3.0-flash の +16.6 ポイントは単軸の数字であり、
AND 判定では +5.8 ポイントにとどまる。単軸では relevance の低下が見えないためで、
[ADR 0018](0018-retire-grounding-gate-as-quality-judge.md) の計測4が「日本語を除くと grounding は上がるが
relevance は下がる」ことを示している。**この分離が効いたのは grounding 軸に対してである。**
