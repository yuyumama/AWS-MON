...
> **導入完了（2026-08-08）。** issue #100〜#105 はすべてクローズし、本節の「実測して記録する」
> としていた項目は結果に置き換えた。テスト件数は 92 → 434
> （shared 44 / api ユニット 218 / api 統合 58 / web 112 / agent 112）。

- **skipされたジョブは required status check で成功扱いになる**（PR #120 / #121 で実測）。
  `agent` / `terraform` が `skipping` のままマージできたため、required から外す必要はなかった。
...
- E2E-P には Cognito のテストユーザーが必要である。[ADR 0006](0006-auth-cognito-cloud-only.md) の
  ...
  この制約はスモーク側で `canGenerateQuestions === false` として表明してある。
  実際、初回のスモーク実行は生成権限が付いた状態を検知して赤になった。
- **CIから SRP ログインできることを確認した**（PR #121）。`amazon-cognito-identity-js` は
  Node からシムなしで SRP を完走でき、ルートの `npm ci` で入るため依存追加も要らなかった。
  懸念していた E2E-P の縮退（`/health` のSHA検証のみ）は行っていない。
- DynamoDB Local の導入による `node` ジョブの増分は**ステップ合計 +約27秒**（うち17秒が
  `terraform init`）。ジョブ全体で 50〜60秒に収まっている。短縮するなら `.terraform` の
  キャッシュが第一候補。
- 本ADRは issue #100〜#105 の再編を伴った。#104 は実行環境の異なる3種の作業が同居していたため
  解体してクローズし、#100 は `auth.ts`（モック）と `answerSession`（統合）に分割した。
